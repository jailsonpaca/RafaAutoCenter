...
function save(chunk) {
  ...
  const printer = fs.readFileSync(`${process.resourcesPath}\\notas\\IMPRESSORA.txt`, 'utf8', (e) => {
    console.log(e);
  })
  console.log(printer);

  var newdate = moment().format('DD_MM_YY-h-mm-ss');
  const pdfPath = `${process.resourcesPath}\\notas\\${newdate}FOLHA.pdf`;
  console.log(pdfPath);

  fs.appendFile(pdfPath, Buffer.from(chunk), function (err) {
    if (err) {
      throw (err);
    } else {
      ...
      /*exec(`cd ${path.dirname(pdfPath)} && ${path.basename(pdfPath)}`, (error, stdout, stderr) => {
            console.log(stdout);
          });
          */
      // return(chunk.length);
  ...
  image2base64(logo) // you can also to use url
    .then(
      (response) => {
        ...
        doc.setFontSize('11');
        var da = moment().format("DD-MM-YYYY");
